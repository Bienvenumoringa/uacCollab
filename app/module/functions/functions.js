...
function attach_item_value(startIndex, endIndex) {
    $(document).on('click', '.delete', function() {
        // Trouver la ligne parente (<tr>) du bouton cliqué
        const $tr = $(this).closest('tr');

        // Sélectionner uniquement les cellules <td> ou <th> qui n'ont pas l'attribut hidden
        const visibleCells = $tr.children('td:not([hidden]), th:not([hidden])').filter(function() {
            return !$(this).attr('hidden'); // Ignorer les cellules cachées
        });

        // Extraire uniquement les valeurs des cellules visibles dans la plage spécifiée
        const data = visibleCells.slice(startIndex, endIndex + 1).map(function() {
            return $(this).text().trim(); // Extraire et nettoyer le texte de chaque cellule
        }).get();

        // Afficher les données extraites dans l'élément avec l'ID 'item'
        $('#idSup').text(data.join(' ')); // Joindre les valeurs avec un séparateur
    });
}
export{show_message,get_value,redirect,save,get_controller_url,handle_display,fill_select,get_file,send,insert,attach_edit_delete_event,attach_item_value};